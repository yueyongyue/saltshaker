...
@login_required(login_url="/account/login/")
def get_history(request, tag, num, search_input):
    cmd_history_list = []
    _u = request.user
    try:
        cmd_history = Command_history.objects.filter(Q(user_id=_u) & Q(command_tag=tag) & Q(command__icontains=search_input)).order_by('-id').all()[:num]
    except Exception as e:
        logger.error(e)
